...
main() {
  ...
  platform=google
  if [[ -n "$platform" ]]; then
    query=$( (echo ) |  rofi -config ~/.config/rofi/custom_scripts/themes/config.simple -dmenu -padding 20 -lines 0 -width 50 -location 0 -font 'Fantasque Sans Mono 20' -p "Query")
